                    ...
                    dbc.Col(
                        [
                            ...
                            dcc.Markdown(
                                ...
                                - _unsupervised anomaly detection_ (i.e. `NOVELTY_DETECTION`) performs novelty and outlier detection. The algorithm uses the user's reference definition as a start (samples where `reference_group==True`) and decides whether a new observation from unlabeled samples belong to the reference or not. We use [`LocalOutlierFactor`](https://scikit-learn.org/stable/auto_examples/neighbors/plot_lof_outlier_detection.html) method with fixed [Bray-Curtis distance](https://en.wikipedia.org/wiki/Bray%E2%80%93Curtis_dissimilarity) metric and fixed number of neighbors for the algorithm (`n_neighbors=1`, the most common number of neighbors in a group of samples, and mean value between 1 and the most common). The features that are used for this model are specified under Dataset Settings feature columns option on Home page. These features are not necessary matching the feature columns used for building the microbiome trajectory. If dashboard user selects novelty detection option, it might not yield the optimal results for ones dataset (fixed parameters cannot generalize accross different datasets). Hence, we suggest to play with the `microbiome-toolbox` methods locally and set these parameters. Here we only display an example/demo for fixed settings (fixed metric and neighbors).  
                                """
                            ),
                            dcc.Markdown(
                                """
                                Below we also analyse the features important in each of the groups. To find the features that differentiate the two groups (reference vs. non-reference group), we train the binary classification model [`RandomForestClassifier`](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestClassifier.html) and perform cross validation with [`GroupShuffleSplit`](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.GroupShuffleSplit.html). The confusion matrix enables the insight on how good the separation between the two groups is.
                                """
                            ),
                            dcc.Markdown(
                                """
