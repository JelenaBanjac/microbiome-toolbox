...
def permuspliner(data, xvar, yvar, category, degree, cases, groups = None, perms = 999, test_direction = 'more', ints = 1000, quiet = False, cut_sparse = 4):
    ...
    if test_direction not in ["more", "less"]:
        raise Exception("Error in test direction option: must be either 'more' or 'less'")
    ...
    if groups is None:
        if len(in_df.category.unique()) > 2:
            raise Exception('More than two groups in category column. Define groups with (groups = c("Name1","Name2"))')
        else:
    ...
    p_v1, _ = np.polyfit(df_v1[xvar], df_v1[yvar], degree, cov=True)           # parameters and covariance from of the fit of 1-D polynom.
    p_v2, _ = np.polyfit(df_v2[xvar], df_v2[yvar], degree, cov=True)           # parameters and covariance from of the fit of 1-D polynom.
    ...
    real_spl_dist = v1_spl_f.set_index('x').join(v2_spl_f.set_index('x'), on="x").reset_index()
    real_spl_dist["abs_dist"] = np.abs(real_spl_dist.var1 - real_spl_dist.var2)  # Measure the real group distance
    real_area = np.sum(real_spl_dist.abs_dist)/ints  # Calculate the area between the groups
    ...
    def spline_permute(randy, ix, perm_output):
        randy_meta = randy.drop_duplicates(subset=cases, keep="first").copy(deep=False)  # Pull out the individual IDs
        randy_meta[case_shuff] = np.random.choice(randy_meta[category].values, size=len(randy_meta[category].values), replace=True, p=None)
        randy_meta = randy_meta[[cases, case_shuff]]
        randy = randy.set_index(cases).join(randy_meta.set_index(cases), on = cases).reset_index()
        ...
        spl_dist  = randy_v1_fit.set_index('x').join(randy_v2_fit.set_index('x'), on="x").reset_index()
        spl_dist["abs_dist"] = np.abs(spl_dist.var1 - spl_dist.var2)  # Measure the real group distance
        
        real_area = np.sum(real_spl_dist.abs_dist)/ints   # Calculate the area between the groups
        
        transfer_perms = spl_dist[["var1", "var2", "abs_dist"]]
        transfer_perms = transfer_perms.rename(columns={"var1": f"v1perm_{ix}", 
                                                        "var2": f"v2perm_{ix}", 
        ...
        perm_area = [np.nansum(spl_dist["abs_dist"])/np.sum(~spl_dist["abs_dist"].isna())]  # Calculate the area between permuted groups
        if ix > 0:
    ...
    result = {"pval": pval, "category_1": v1, "category_2": v2,
...
def permuspliner_plot_permdistance(data, xlabel=None, ylabel=""):
    ...
    dists = pd.melt(dists, id_vars="x", var_name="permutation", value_name="permuted_distance")
    ...
    for p in dists.permutation.unique():
        ax.plot(dists[dists.permutation==p]["x"].values, dists[dists.permutation==p]["permuted_distance"].values, alpha=alpha_level, solid_capstyle="butt", c="k")
    
    true_dist = data['true_distance']
    ax.plot(true_dist["x"].values, true_dist["abs_dist"].values, color="r")
    plt.show()
    
def permuspliner_plot_permsplines(data = None, xvar=None, yvar=None, xlabel="", ylabel=""):
    ...
    permsplines = pd.melt(permsplines, id_vars="x", var_name="permutation", value_name="y.par")
    ...
    for p in permsplines.permutation.unique():
        ax.plot(permsplines[permsplines.permutation==p]["x"].values, permsplines[permsplines.permutation==p]["y.par"].values, alpha=alpha_level, solid_capstyle="butt", c="k")
    
    true_data["y"] = true_data.apply(lambda row: row["var1"] if row["Group"]==var_1 else row["var2"], axis=1)
    sns.lineplot(x="x", y="y", data=true_data, hue="Group", ci=None, palette=["r","b"], ax=ax)
    plt.show()
...
def regliner(df2countries_stats, mapping):
    ...
    dfts["Input*Condition"] = dfts.apply(lambda x: mapping[x["Condition"]]*x["Input"], axis=1)
